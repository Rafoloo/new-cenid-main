...
const ConsultaSchema = z
  .object({
    ...
    dataConsulta: z.string().refine((date) => {
  ...
  .refine((data) => !data.precisaAcompanhante || (data.nomeAcompanhante && data.nomeAcompanhante.length > 0), {
    message: "O nome do acompanhante é obrigatório quando precisa de acompanhante",
    path: ["nomeAcompanhante"],
  })
  .refine((data) => !data.consultaRemota || (data.linkConsultaRemota && data.linkConsultaRemota.length > 0), {
    ...
    <Card className="max-w-5xl mx-auto my-8 shadow-md rounded-lg border border-gray-100">
      <CardHeader className="bg-teal-100 p-4">
        <div className="flex items-center space-x-4">
          ...
          <CardTitle className="text-2xl font-semibold text-teal-800">Cadastro de Consulta</CardTitle>
        </div>
      </CardHeader>
      <CardContent className="p-6">
        <div className="mb-6">
          <h2 className="text-lg font-medium text-teal-700 mb-4">Selecione a Especialidade*</h2>
          ...
          {form.formState.errors.especialidade && (
            <p className="text-sm text-red-500 mt-2">{form.formState.errors.especialidade.message}</p>
          )}
        </div>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                          ...
                          <PopoverTrigger asChild>
                            ...
                            >
                              ...
                              {field.value ? (
                                ...
                                <span className="text-teal-800">Escolha uma data</span>
                              )}
                            </Button>
                          </PopoverTrigger>
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <div className="relative">
                          <Input type="time" placeholder="00:00" className="border-gray-300" {...field} />
            ...
            <section className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                {" "}
                {/* Alterado para 5 colunas */}
                {/* Sala de Atendimento - Ocupa 3 colunas */}
                ...
                {/* Consulta Remota - Ocupa 2 colunas */}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem className="flex flex-row items-center space-x-2 space-y-0">
                        ...
                        <FormLabel className="mt-1">Consulta Remota</FormLabel> {/* Ajuste fino de alinhamento */}
                      </FormItem>
                    )}
                  />
                </div>
                {/* Link da Consulta (condicional) */}
                <div
                  className={cn(
                    "transition-all duration-300 ease-in-out",
                    form.watch("consultaRemota")
                      ? "max-h-40 opacity-100 md:col-span-5"
